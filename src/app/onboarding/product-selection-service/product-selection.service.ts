...
import { ProductSelection, ProductSelectionMaintanence } from './product-selection-service.model';
import { environment } from 'src/environments/environment';
import { ProductInfo, ProductInfoMaintanence } from './productInfo.model';
@Injectable()
export class ProductSelectionService implements BaseFpxDataService<any> {
  constructor(private _httpProvider: HttpProviderService) { }
  findAll(criteriaQuery: CriteriaQuery, httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindAllFn<ProductInfoMaintanence> {
    return () => {
      ...
      return this._httpProvider
        ...
        .pipe(
          map(
            (res: IHttpSuccessPayload<ProductInfoMaintanence>) => {
              return {
  ...
  create(payload: ProductSelection): CreateFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
  ...
  findByKey(key: ProductSelection): FindByKeyFn<ProductSelection> {
    return () => {
      const httpRequest = new HttpRequest();
  ...
  modify(payload: ProductSelection): ModifyFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
