...
@Component({
  selector: 'app-prepaidcard-spending-summary',
  templateUrl: './prepaidcard-spending-summary.component.html',
  styleUrls: ['./prepaidcard-spending-summary.component.scss']
})
export class PrepaidcardSpendingSummaryComponent implements OnInit, OnChanges {
  ...
  protected appConfig: AppConfigService = inject(AppConfigService);
	...
	@Output() currencyChange:EventEmitter <any> = new EventEmitter<  any|null>();
  totalSummary: number = 0;
  prevDisable: boolean = false;
  nextDisable: boolean = false;
  selectedCurrency: any;

  private _prepaidcardDetails: any = [];
  @Input() set prepaidcardDetails(_data:any){
    this._prepaidcardDetails = _data;
    this.selectedCurrency = _data?.balanceDetails[0].currency
  }
  get prepaidcardDetails(){
    return this._prepaidcardDetails;
  }
  ...
  checkButtonState() {
    ...
    this.spendDate.emit({
      ...
      'currentSpendDate': this.currentSpendDate
    });
