...
import { Blockeddc } from './blockeddc.model';

@Injectable({
  providedIn: 'root',
})
export class BlockeddcService  implements BaseFpxDataService<any> {
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   findByKey(key: Blockeddc,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindByKeyFn<Blockeddc|null> {
    return () => {
      const httpRequest = new HttpRequest();
       httpRequest.setResource('/blockeddc/{cardRefNumber}');
 ...
 lookup(key: any,httpOption : Map<keyof FpxIHttpOption, Map<string, any>> = new Map(), criteriaQuery?: CriteriaQuery | undefined): LookUpFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
      httpRequest.setMethod('GET');
      httpRequest.setResource('/blockeddc');
      httpRequest.setContextPath('DebitCards');
