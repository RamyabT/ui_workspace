...
export class RetailDcTransactionDownloadFilterState extends BaseFpxComponentState {
...
@Injectable()
export class RetailDcTransactionDownloadFilterHelper extends BaseFpxFormHelper<RetailDcTransactionDownloadFilterState>{
  ...
  constructor(
    ...
    private retailDcTransactionDownloadFilterService: RetaildctransactiondownloadfilterService, private _httpProvider: HttpProviderService, private _router: Router) {
    super(new RetailDcTransactionDownloadFilterState());
  }
  ...
  public onDownloadClick: BaseFpxControlEventHandler = (payload: any) => {
    ...
    let cardRefNumber = this.cardData.cardRefNumber;
    ...
    if (this.getValue('downloadFileFormat') == 1) {
      this.commonService.downloadDCTransactionStatement(criteriaQuery).subscribe({
        next: (response: any) => {
          if (this.deviceDetectorService.isHybrid()) {
            this._fileOpener.openPDF(
              response,
              "application/pdf",
              "DebitCardTransactionHistory.pdf"
            );
          } else {
            ...
            const fileName = "DebitCardTransactionHistory.pdf";
      ...
      this.commonService.downloadDCTransactionStatementCSV(criteriaQuery).subscribe({
        next: (response: any) => {
          if (this.deviceDetectorService.isHybrid()) {
            this._fileOpener.openPDF(
              response,
              "text/csv",
              "DebitCardTransactionHistory.csv"
            );
          } else {
            ...
            const downloadLink = document.createElement("a");
            downloadLink.href = documentURL;
            const fileName = "DebitCardTransactionHistory.csv";
            downloadLink.download = fileName;
          }
        }
      });
