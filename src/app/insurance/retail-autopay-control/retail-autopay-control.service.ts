...
export class RetailAutopayControlService  implements BaseFpxDataService<any> {
constructor(private _httpProvider: HttpProviderService,private translateService:TranslateService) {}
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   lookup(key: any): LookUpFn<any> {
    return () => {
    let Data=[];
	 Data.push({'id':'0','text':this.translateService.instant('Yes')});
	 Data.push({'id':'1','text':this.translateService.instant('No')});
    const lookupInfo = {Data};
    const retailAutopayControlList$ = new Observable(observer => {
      observer.next(lookupInfo.Data);
      observer.complete();
    });
    return retailAutopayControlList$;
    };
  }
