...
export class InsuranceAdsService extends BaseFpxFunctionality implements BaseFpxDataService<unknown> {
    constructor(
    ...
    fetchInsuranceAds(payload: any):Observable<any> {
        const httpRequest = new HttpRequest();
        httpRequest.setMethod('GET');
        httpRequest.setResource(`/${payload.servicePath}`);
        httpRequest.addQueryParameter('lookup', 1);
        httpRequest.addHeaderParamter('serviceCode', payload.serviceCode);
        
        return this._httpProvider.invokeRestApi(httpRequest).pipe(map((res: IHttpSuccessPayload<any>) => res?.body?.Data));  }
}