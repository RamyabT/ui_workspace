...
@Component({
  selector: 'app-insurance-confirmation-receipt-form',
  templateUrl: './insurance-confirmation-receipt-form.component.html',
  styleUrls: ['./insurance-confirmation-receipt-form.component.scss']
})

export class InsuranceConfirmationReceiptFormComponent extends BaseFpxFunctionality {
  ...
  ngOnInit(): void {
    console.log('Insurance: confirmation receipt');
  }
    ...
    this._transfersService
      ...
      .subscribe((res) => {
        if (res.favpayments) {
          ...
          fpxModal.setData({
            title: "insuranceConfirmationForm.favSuccessAlert.title",
            message: "insuranceConfirmationForm.favSuccessAlert.message"
          });
          ...
          fpxModal.setBackDropClass('dep-popup-back-drop');
          fpxModal.setData({
            title: "insuranceConfirmationForm.favFailureAlert.title",
  ...
  public setPageDependency(payload: any): void {
    ...
    if (payload?.status == 'FAILUR') {
      ...
      if (payload?.requestStatus == "ErrorEnd") {
        ...
        this.failedPayment = "insuranceConfirmationForm.defaultErrorMessage";
  ...
  gotoModule(module: string) {
    if (this.deviceService.isMobile()) {
      this._router.navigate(['insurance-space']);
    }
    else {
