...
@Component({
  selector: "app-retail-cc-transfer-history-template",
  templateUrl: "./retail-cc-transfer-history-template.component.html",
  styleUrls: ["./retail-cc-transfer-history-template.component.scss"],
})
export class RetailCcTransferHistoryTemplateComponent extends DepPanningComponent implements OnInit {
  ...
  editRaiseDispute(selectedData: TempScheduleRep) {
    console.log(selectedData);
    this._appConfig.setData('ccTransferHistory',selectedData);
    this._router.navigate(['cards-space','entry-shell','credit-cards','retail-cc-raise-dispute-form'],{
      queryParams: {
        'serviceCode': selectedData.serviceCode,
