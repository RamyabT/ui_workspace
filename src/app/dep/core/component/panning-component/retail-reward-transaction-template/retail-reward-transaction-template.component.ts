...
@Component({
 selector: 'app-retail-reward-transaction-template',
  templateUrl: './retail-reward-transaction-template.component.html',
  styleUrls: ['./retail-reward-transaction-template.component.scss'],
})
export class RetailRewardTransactionTemplateComponent extends DepPanningComponent {
  requestData: any;
  constructor(
  ...
  ondeleted(event:any ,selectedData: any) {
    this.requestData = selectedData;
  ...
  flashCardModelAfterClose: FpxModalAfterClosed = (payload: any, addtionalData: any) => {
    ...
    if (payload == 1) {
      if (this._appConfig.hasData('walletActionPublisher$')) {
        this._appConfig.getData('walletActionPublisher$').subject.next({
