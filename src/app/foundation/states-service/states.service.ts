...
export class StatesService  implements BaseFpxDataService<any> {
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   findByKey(key: States,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindByKeyFn<States|null> {
    return () => {
      const httpRequest = new HttpRequest();
       httpRequest.setResource('/states/{countryCode}/{stateCode}');
       httpRequest.addPathParameter('countryCode', key.countryCode);
       httpRequest.addPathParameter('stateCode', key.states);
      httpRequest.setMethod('GET');
      return this._httpProvider
        .invokeRestApi(httpRequest,httpOption)
        .pipe(map((res: IHttpSuccessPayload<any>) => res.body?.states ?? null),catchError((err:any) => {
              return of(null)
            }));
      };
  }
 ...
 lookup(key: any,httpOption : Map<keyof FpxIHttpOption, Map<string, any>> = new Map(), criteriaQuery?: CriteriaQuery | undefined): LookUpFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
      httpRequest.setMethod('GET');
      httpRequest.setResource('/states');
