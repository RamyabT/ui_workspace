...
export class GenderControlService  implements BaseFpxDataService<any> {
constructor(private _httpProvider: HttpProviderService,private translateService:TranslateService) {}
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   lookup(key: any): LookUpFn<any> {
    return () => {
    let Data=[];
    Data.push({'id':'M','text':this.translateService.instant('gender-control.M')});
	 Data.push({'id':'F','text':this.translateService.instant('gender-control.F')});
   Data.push({'id':'O','text':this.translateService.instant('gender-control.O')});
