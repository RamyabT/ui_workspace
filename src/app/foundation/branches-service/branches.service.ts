...
export class BranchesService  implements BaseFpxDataService<any> {
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   findByKey(key: Branches,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindByKeyFn<Branches|null> {
    return () => {
      const httpRequest = new HttpRequest();
      httpRequest.setResource('/branches/{branchCode}');
      httpRequest.setContextPath('Accounts');
       httpRequest.addPathParameter('branchCode', key.branchCode);
      httpRequest.setMethod('GET');
      return this._httpProvider
        .invokeRestApi(httpRequest,httpOption)
        .pipe(map((res: IHttpSuccessPayload<any>) => res.body?.branches ?? null),catchError((err:any) => {
              return of(null)
            }));
      };
  }
