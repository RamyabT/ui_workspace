  ...
  findByKey(key: Cobaddressinfo,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindByKeyFn<Cobaddressinfo|null> {
    return () => {
      ...
      return this._httpProvider
        ...
        .pipe(map((res: IHttpSuccessPayload<any>) => res.body?.cobaddressinfo ?? null), map((res:any) => {
          return res ? {...res,  country:res.country?.countryCode, state:res.state?.stateCode,} : null
        }),catchError((err:any) => {
              return of(null)
            }));
