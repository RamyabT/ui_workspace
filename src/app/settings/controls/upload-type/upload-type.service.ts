...
export class UploadTypeService  implements BaseFpxDataService<any> {
constructor(private _httpProvider: HttpProviderService,private translateService:TranslateService) {}
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   lookup(key: any): LookUpFn<any> {
    return () => {
    let Data=[];
	 Data.push({'id':'1','text':this.translateService.instant('upload-type.1')});
	 Data.push({'id':'2','text':this.translateService.instant('upload-type.2')});
    const lookupInfo = {Data};
    const uploadTypeList$ = new Observable(observer => {
