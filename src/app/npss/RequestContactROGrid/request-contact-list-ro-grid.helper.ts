...
@Injectable()
export class NPSSRequestContactViewingROGRIDHelper extends BaseFpxRoGridHelper {
  customPayload: any = {
  ...
  private retailContactDetailRoGridView: BaseFpxRoGridHandleAction = (
    ...
    let iban: any;
    let currency: any;
    // let mobileNumber = data.phoneNumbers[0].value.substring(3,)
        ...
        next: (res: any) => {
          ...
          if (res?.bankUserId!='') {
            for (i; i < res.accountDetails.length; i++) {
              ...
              this._appconfig.setData('npssRequestMoney', this.customPayload);
