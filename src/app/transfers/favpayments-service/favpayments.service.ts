...
import { map, Observable, of,catchError, Subject, BehaviorSubject } from 'rxjs';
...
isFavTransferAvailable = new Subject<boolean>();
refreshFavTransaferSub$ = new EventEmitter<Favpayments | null>();
refreshViewAllFavTransaferSub$ = new EventEmitter<Favpayments | null>();
// refreshFavTransafer$ = this.refreshFavTransaferSub$.asObservable();
 constructor(private _httpProvider : HttpProviderService) { }

  refreshFavTransfer(favpayments: Favpayments){
    this.refreshFavTransaferSub$.emit(favpayments); 
  }
  refreshViewAllFavTransfer(favpayments: Favpayments){
    this.refreshViewAllFavTransaferSub$.emit(favpayments); 
  }
  ...
  modify(payload: Favpayments,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): ModifyFn<any> {
   ...
   findAll(criteriaQuery: CriteriaQuery,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindAllFn<FavpaymentsMaintanence> {
    return () => {
      ...
      httpRequest.setResource('/favpayments');
      httpRequest.setMethod('GET');
      httpRequest.addHeaderParamter('serviceCode','RETAILTRANSFERFAVPAYMENTS');
