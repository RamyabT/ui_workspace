...
constructor(private _httpProvider: HttpProviderService,private translateService:TranslateService) {}
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   lookup(key: any): LookUpFn<any> {
    return () => {
    let Data=[];
	 Data.push({'id':'1','text':this.translateService.instant('transfer-type-control.1')});
	 Data.push({'id':'2','text':this.translateService.instant('transfer-type-control.2')});
	 Data.push({'id':'3','text':this.translateService.instant('transfer-type-control.3')});
    const lookupInfo = {Data};
    const transferTypeList$ = new Observable(observer => {
