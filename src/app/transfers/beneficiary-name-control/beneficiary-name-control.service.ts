...
export class BeneficiaryNameControlService  implements BaseFpxDataService<any> {
constructor(private _httpProvider: HttpProviderService,private translateService:TranslateService) {}
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   lookup(key: any): LookUpFn<any> {
    return () => {
    let Data=[];
	 Data.push({'id':'key','text':this.translateService.instant('beneficiary-name-control.key')});
    const lookupInfo = {Data};
    const beneficiaryNameControlList$ = new Observable(observer => {
      observer.next(lookupInfo.Data);
      observer.complete();
    });
    return beneficiaryNameControlList$;
    };
  }
