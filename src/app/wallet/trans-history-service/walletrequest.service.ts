...
import { Transactionreceivemaintainence } from './walletreceive.model';
import { Transactionrequestemaintainence } from './walletrequest.model';
import { AppConfigService } from '@dep/services';
@Injectable({
  providedIn: 'root'
})

export class WalletrequestService implements BaseFpxDataService<any> {
    ...
    return () => {
     ...
     httpRequest.addQueryParameter('walletId', this._appConfig.getData('walletID'));
      ...
      return this._httpProvider
        ...
        .pipe(
          map(
            (res: IHttpSuccessPayload<Transactionrequestemaintainence>) =>{
             return{
              data:res.body.walletrequestedinfo?.walletrequestedinfo || [],
