...
import { walletrequestedinfo } from '../trans-history-service/walletrequest.model';
import { WalletrequestService } from '../trans-history-service/walletrequest.service';
  

@Injectable()
export class WalletRequestedSummaryRoGridHelper extends BaseFpxRoGridHelper {
  constructor(
    private _router: Router,
    private _httpProvider: HttpProviderService,
    private _currencyFormatter: FpxCurrenyFormatterPipe,
    @Inject(MAT_DIALOG_DATA) private _dialogData: any,
    private _WallethistroyService: WalletrequestService,
  ...
  private transformScheduleType: GridTransformFn<walletrequestedinfo> = (payload: walletrequestedinfo) => {
  ...
  override doPreInit(): void {
    ...
    this.setNgTemplateName('walletRequestListTmplt');
  ...
  override postFindallInterceptor = (payload: any) => {
    let rowData: walletrequestedinfo[] = [];
    let _date = "";
    ...
    payload.data.forEach((element: any) => {
      let _d = element.requestedDate.split(' ')[0];
      if (_date != _d) {
        _date = _d;
        let rowGroup: any = {
          rowGroupTitle: _date
