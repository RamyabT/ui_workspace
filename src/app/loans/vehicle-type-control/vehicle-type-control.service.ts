...
export class VehicleTypeControlService implements BaseFpxDataService<any> {
  ...
  lookup(key: any): LookUpFn<any> {
    return () => {
      let Data = [];
      Data.push({ 'id': '0', 'text': "2-wheeler" });
      Data.push({ 'id': '1', 'text': "4-wheeler" });
      const lookupInfo = { Data };
      const VehicleTypeControlList$ = new Observable(observer => {
        observer.next(lookupInfo.Data);
        observer.complete();
      });
      return VehicleTypeControlList$;
    };
  }
