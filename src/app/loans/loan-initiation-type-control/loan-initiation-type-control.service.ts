...
export class LoanInitiationTypeControlService implements BaseFpxDataService<any> {
  ...
  lookup(key: any): LookUpFn<any> {
    return () => {
      let Data = [];
      Data.push({ 'id': 'N', 'text': "New Purchase" });
      Data.push({ 'id': 'R', 'text': "Refinance" });
      const lookupInfo = { Data };
      const LoanInitiationTypeControlList$ = new Observable(observer => {
        observer.next(lookupInfo.Data);
        observer.complete();
      });
      return LoanInitiationTypeControlList$;
    };
  }
