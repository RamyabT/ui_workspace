...
export class VehicleStatusControlService implements BaseFpxDataService<any> {
  ...
  lookup(key: any): LookUpFn<any> {
    return () => {
      let Data = [];
      Data.push({ 'id': '0', 'text': "New" });
      Data.push({ 'id': '1', 'text': "Second-hand" });
      const lookupInfo = { Data };
      const vehicleStatusControlList$ = new Observable(observer => {
        observer.next(lookupInfo.Data);
        observer.complete();
      });
      return vehicleStatusControlList$;
    };
  }
