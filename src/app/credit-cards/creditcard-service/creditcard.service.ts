...
export class CreditcardService implements BaseFpxDataService<any> {
 ...
 constructor(private _httpProvider : HttpProviderService) { }
  ...
  fetchCreditCardRegistrationStatus(): Observable<any> {
    const httpRequest = new HttpRequest();
    httpRequest.setMethod('GET');
    httpRequest.setResource('/cardregistrationstatus');
    httpRequest.addHeaderParamter('serviceCode', 'RETAILCARDREGSTATUS');
    httpRequest.setContextPath('CreditCards');
    return this._httpProvider.invokeRestApi(httpRequest).pipe(
      map((res: IHttpSuccessPayload<any>) =>
        res.body ?? null
