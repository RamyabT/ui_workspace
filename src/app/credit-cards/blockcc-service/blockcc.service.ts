...
import { Blockcc } from './blockcc.model';

@Injectable({
  providedIn: 'root',
})
export class BlockccService  implements BaseFpxDataService<any> {
  ...
  modify(payload: any): ModifyFn<any> {
   ...
   findByKey(key: Blockcc,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): FindByKeyFn<Blockcc|null> {
    return () => {
      const httpRequest = new HttpRequest();
       httpRequest.setResource('/blockcc/{cardRefNumber}');
 ...
 lookup(key: any,httpOption : Map<keyof FpxIHttpOption, Map<string, any>> = new Map(), criteriaQuery?: CriteriaQuery | undefined): LookUpFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
      httpRequest.setMethod('GET');
      httpRequest.setResource('/blockcc');
