...
import { map, Observable, of,catchError } from 'rxjs';
 import { Dcpinrequest, DcpinrequestMaintanence } from './ccpinvalidation.model';
@Injectable()
export class CcpinvalidationService implements BaseFpxDataService<any> {
 constructor(private _httpProvider : HttpProviderService) { }
  create(payload: Dcpinrequest,httpOption: Map<keyof FpxIHttpOption, Map<string, any>> = new Map()): CreateFn<any> {
    return () => {
      const httpRequest = new HttpRequest();
      httpRequest.setMethod('POST');
      httpRequest.setResource('{cardRefNumber}/dcpinrequest');
      httpRequest.addPathParameter('cardRefNumber', payload.cardRefNumber);
      httpRequest.setContextPath('CreditCards');
      let bodyContent = {"ccPinVerify":payload};
