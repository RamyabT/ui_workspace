...
export class PfmTransactionModifyFormState extends BaseFpxComponentState {
...
@Injectable()
export class PfmTransactionModifyFormHelper extends BaseFpxFormHelper<PfmTransactionModifyFormState>{

   constructor( private pfmTransactionModifyFormService: PfmtransactionService, private _httpProvider : HttpProviderService,private _router: Router) 
    {
        super(new PfmTransactionModifyFormState());
    }
 ...
 public handleFormOnLoad(){
  ...
  this.pfmTransactionModifyFormService.findByKey(key)().subscribe((res: any) => {
      if (res) {
        this.setValue('externalReferenceNumber', res?.inventoryNumber);
        this.setValue('paymentDate', res?.paymentDate);
        this.setValue('merchantCode', res?.merchantCode?.merchantName);
        ...
        this.setValue('pfmSubCategory', res?.subCategoryCode?.subCategoryCode);
      }
    },
  )
  ...
  this.setReadonly('merchantCode',true);
