...
@Component({
  selector: 'app-deposits-link-accounts-list',
  templateUrl: './deposits-link-accounts-list.component.html',
  styleUrls: ['./deposits-link-accounts-list.component.scss']
})
export class DepositsLinkAccountsListComponent implements OnInit {
  ...
  onClickAviso(event: any) {
    ...
    this._appConfig.getData('refreshAvisoDtl$').subject.next({
      refreshAvisoDtl: true,
    });
    return;
  }

  onClickQtrade(event: any) {
