...
@Component({
  selector: 'app-fb-confirmation-receipt-form',
  templateUrl: './fb-confirmation-receipt-form.component.html',
  styleUrls: ['./fb-confirmation-receipt-form.component.scss']
})

export class FbConfirmationReceiptFormComponent extends BaseFpxFunctionality {
    ...
    this._transfersService
      ...
      .subscribe((res) => {
        if (res.favpayments) {
          ...
          fpxModal.setData({
            title: "fbConfirmationForm.favSuccessAlert.title",
            message: "fbConfirmationForm.favSuccessAlert.message"
          });
          ...
          fpxModal.setBackDropClass('dep-popup-back-drop');
          fpxModal.setData({
            title: "fbConfirmationForm.favFailureAlert.title",
  ...
  public setPageDependency(payload: any): void {
    ...
    if (payload?.status == 'FAILUR') {
      ...
      if (payload?.requestStatus == "ErrorEnd") {
        ...
        this.failedPayment = "fbConfirmationForm.defaultErrorMessage";
  ...
  gotoModule(module: string) {
    if (this.deviceService.isMobile()) {
      this._router.navigate(['fb-space']);
    }
    else {
      this._router.navigate(['fb-space', 'fb']);
    }
  }
  ...
  onShareClick() {
    ...
    this._shareInfo.shareInfo(confirmationReceiptInfo, this.translate.instant('fbConfirmationReceipt.shareSuccess'));
  }
